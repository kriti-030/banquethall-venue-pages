...
const outDir = path.join(root, "FINAL_JSX_VENUES");

const VENUES = [
  { source: "LeMeridienNewDelhi.jsx", output: "LeMeridienNewDelhi.jsx", component: "LeMeridienNewDelhi" },
  { source: "CrownePlazaNewDelhiOkhla.jsx", output: "CrownePlazaNewDelhiOkhla.jsx", component: "CrownePlazaNewDelhiOkhla" },
  { source: "TheSuryaaNewDelhi.jsx", output: "TheSuryaaNewDelhi.jsx", component: "TheSuryaaNewDelhi" },
  { source: "GoldfinchHotelDelhiNCR.jsx", output: "GoldfinchHotelFaridabad.jsx", component: "GoldfinchHotelFaridabad" },
  { source: "RadissonBluMarinaDelhi.jsx", output: "RadissonBluMarinaConnaughtPlace.jsx", component: "RadissonBluMarinaConnaughtPlace" },
  { source: "FortuneSelectGlobalGurgaon.jsx", output: "FortuneSelectGlobalGurgaon.jsx", component: "FortuneSelectGlobalGurgaon" },
  { source: "ParkInnRadissonLajpatNagar.jsx", output: "ParkInnByRadissonLajpatNagar.jsx", component: "ParkInnByRadissonLajpatNagar" },
  { source: "HiltonGardenInnSaket.jsx", output: "HiltonGardenInnSaket.jsx", component: "HiltonGardenInnSaket" },
];

const SHARED_IMPORTS = `import { NavLink, useLocation } from "react-router-dom";
...
import customerimage from "../src/assets/customerimage.svg";
`;
...
function extractVenueData(sourceContent) {
  const imageImports = sourceContent
    .split("\n")
    .filter((line) => /^import venue\d/.test(line))
    .map((line) => line.replace('from "../assets/', 'from "../src/assets/'))
    .join("\n");

  const reviewsStart = sourceContent.indexOf("const reviews = ");
  const imagesStart = sourceContent.indexOf("const images = ");
  const venueStart = sourceContent.indexOf("const venue = ");
  const componentStart = sourceContent.search(/\nconst [A-Za-z]+ = \(\) =>/);

  if (reviewsStart === -1 || imagesStart === -1 || venueStart === -1 || componentStart === -1) {
    throw new Error("Could not parse venue data from source file");
  }

  const reviews = sourceContent
    .slice(reviewsStart + "const reviews = ".length, imagesStart)
    .trim()
    .replace(/;\s*$/, "");
  const images = sourceContent
    .slice(imagesStart + "const images = ".length, venueStart)
    .trim()
    .replace(/;\s*$/, "");
  const venue = sourceContent
    .slice(venueStart + "const venue = ".length, componentStart)
    .trim()
    .replace(/;\s*$/, "");

  return { imageImports, reviews, images, venue };
}
...
for (const { source, output, component } of VENUES) {
  const sourcePath = path.join(pagesDir, source);
  const sourceContent = fs.readFileSync(sourcePath, "utf8");
  const { imageImports, reviews, images, venue } = extractVenueData(sourceContent);

  const fileContent = `${SHARED_IMPORTS}
${imageImports}
...
const ${component} = () => {${MAIN_COMPONENT_BODY}
};
...
console.log("\nDone. Created", created.length, "files in", outDir);
